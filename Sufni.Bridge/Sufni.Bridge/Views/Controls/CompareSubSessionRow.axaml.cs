...
/// <summary>
/// Compare-mode counterpart to <see cref="SubSessionRow"/>: a single row inside the
/// expanded sub-session list of a combined session, but tapping the row toggles compare
/// selection instead of opening the session. Renders itself recursively so combined
/// sessions nested inside other combined sessions (up to the max combine depth) also get
/// a chain-icon toggle and their own indented sub-session list.
/// </summary>
public partial class CompareSubSessionRow : UserControl
{
    ...
    public static readonly StyledProperty<int> LevelProperty =
        AvaloniaProperty.Register<CompareSubSessionRow, int>(nameof(Level), 1);
    ...
    public static readonly StyledProperty<int> NextLevelProperty =
        AvaloniaProperty.Register<CompareSubSessionRow, int>(nameof(NextLevel), 2);

    /// <summary>Level to assign to a nested CompareSubSessionRow one level deeper than this one.</summary>
    ...
    public static readonly StyledProperty<Thickness> RowPaddingProperty =
        AvaloniaProperty.Register<CompareSubSessionRow, Thickness>(nameof(RowPadding));
    ...
    public static readonly StyledProperty<Thickness> SeparatorMarginProperty =
        AvaloniaProperty.Register<CompareSubSessionRow, Thickness>(nameof(SeparatorMargin));
