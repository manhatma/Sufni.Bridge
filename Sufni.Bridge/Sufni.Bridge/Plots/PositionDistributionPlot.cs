...
public class PositionDistributionPlot(Plot plot, SuspensionType type) : TelemetryPlot(plot)
{
    ...
    private void AddStatistics(TelemetryData telemetryData, double yRangeTop)
    {
        ...
        var p99Percentage = mx > 0 ? statistics.P99 / mx * 100.0 : 0.0;
        ...
        Plot.Add.VerticalLine(statistics.P99, 2f, Color.FromHex("#FFD700"), LinePattern.Dashed);
        ...
        AddLabel("99th", statistics.P99, yRangeTop * 0.95, -8, 0, Alignment.MiddleRight);

        var avgString = $"Avg:  {statistics.Average:F1} mm ({avgPercentage:F1}%)";
        var p99String = $"99th: {statistics.P99:F1} mm ({p99Percentage:F1}%)";
