...
public partial class CompareSessionsViewModel : ViewModelBase
{
    ...
    private static SessionStats? BuildSessionStats(TelemetryData data, SuspensionType type)
    {
        ...
        return new SessionStats(
            travel, velocity,
            compVels.Count > 0 ? compVels.Percentile(99) : 0.0,
            rebVels.Count > 0 ? -rebVels.Percentile(99) : 0.0,
            maxTravel, bands);
    }
    ...
    private static List<CompareTableRow> BuildSummaryRows(List<SessionStats?> statsList, List<SessionViewModel> sessions, SuspensionType type)
    {
        var rows = new List<CompareTableRow>
        {
            ...
            new("Pos [99th, %]", statsList.Select(s => s is null ? "-" : FormatTravel(s.Travel.P99, s.MaxTravel)).ToList()),
            ...
            new("Comp [99th, mm/s]", statsList.Select(s => s is null ? "-" : FormatVelocity(s.Comp99th)).ToList()),
            new("Reb [99th, mm/s]", statsList.Select(s => s is null ? "-" : FormatVelocity(s.Reb99th)).ToList()),
